/// <reference types="node" />
import { SignedMessageNEP0413, SignMessageOptionsNEP0413 } from "./types";
export declare class AuthPayload implements SignMessageOptionsNEP0413 {
    readonly message: string;
    readonly recipient: string;
    readonly nonce: Buffer;
    readonly callbackUrl?: string | undefined;
    readonly tag: number;
    constructor({ message, nonce, recipient, callbackUrl }: SignMessageOptionsNEP0413);
}
export declare const authPayloadSchema: Map<typeof AuthPayload, {
    kind: string;
    fields: ((string | number[])[] | (string | {
        kind: string;
        type: string;
    })[])[];
}>;
export declare function authenticate(request: SignMessageOptionsNEP0413, result: SignedMessageNEP0413): Promise<boolean>;
export declare function verifySignature(request: SignMessageOptionsNEP0413, result: SignedMessageNEP0413): boolean;
export declare function verifyFullKeyBelongsToUser(payload: SignedMessageNEP0413): Promise<boolean>;
